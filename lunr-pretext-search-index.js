...
},
{
  ...
  "body": " Inferential Statistics   Inferential statistics is the branch of statistics that enables us to draw conclusions about a population based on data from a sample. Unlike descriptive statistics, which summarize observed data, inferential statistics use probability theory to make generalizations about the population from which the sample is drawn.  The core concepts of inferential statistics include:  Population : The entire group of interest.  Sample : A subset of the population that is observed.   Parameter : A numerical characteristic of the population (e.g., population mean , variance ).     Statistic : A statistic is a random variable such as sample mean, whose numerical value can be calculated from a sample of the population. The average and standard deviation of the statistic are used to draw conclusions about the population.     So, you will have to pay attention to the same name used in two context, e.g., population ( true ) mean , which would be a single number, usually unknown or even unkowable, versus sample mean, , which is an average over data points of the sample and varies from sample to sample. If outcome of each trial is either or , then the sample mean would be equal to the proportion of 's in the sample. The proportion of 's is the frequentsts' probability of finding in any trial. Whether we estimate or , the analytic procedure is same.    Point Estimation   Point estimation involves using a sample statistic to estimate a population parameter. For example, the sample mean is used to estimate the population mean , and the sample proportion estimates the population proportion . A good point estimator should be unbiased (its expected value equals the true parameter) and have low variance (it is precise).  For instance, if we measure the heights of 50 randomly selected adults and find a sample mean of , we use as a point estimate for the population mean height . However, point estimates do not convey uncertainty, which is why confidence intervals are often used to provide a range of plausible values, which we turn next.    Confidence Intervals  Suppose we want to estimate the mean height of all adult men. Clearly, we would not have the time or resources to measure heights of all men and then find their mean. Instead, we may randomly select men and measure their heights. From this sample we can compute the value of the sample mean of the data we collected. This is not telling us if our answer is anywhere close to the true . The theory based on the Central Limit Theorem can be used to provide probability that if is large enough, we can be certain to a point that the true will be within a confidence interval around the sample mean. where the lower and upper values of th confidence interval are computed from the data in the sample. The larger the , the tighter will be the interval. Another way of writing this is with    "
},
{
