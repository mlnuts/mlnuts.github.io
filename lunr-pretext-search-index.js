...
},
{
  ...
  "number": "2.2",
  "title": "Regression",
  "body": " Regression   The task of predicting a continuous variable or an interval variable is called regression . We usually have a collection of data from which we are to learn a function which we can use to predict corresponding to a new .  There are two common error functions that we minimze.    Squared Error:       Absolute Error:       Of these, squared error is more common, which we will discuss here. The error for an arbitrary point in the space of will be Therefore, the equation for optimal becomes Setting the functional derivative of the integral on the right side to zero shoudl give us the extremum function, say . This will give us (Think of a regular partial derivative with as an independent variable.) That is the extremum of the squared loss occurs at \\tilde{f}. Is it mimimum? To show that it is a minimum, try finding the second derivative and use the expectation symbol to hide away the integration. That means the extremum value is actualy a minimum. Thus, the optimal funtion that minimizes the squared error is the one that maps to the mean value of in the conditional probability . Hence, in terms of the random variables, the optimal function is This is a theretical result of the prediction function. A very common approch to regression is to assume to be a constant bias plus a linear function of , then you can use points in the dataset to estimate the optimal bias and slope(s), which are collectively called parameters of the model and will often be denoted by the letter . We would tend to write the optimal function by or which we will use to make y-prediction for the new point .    The Simple Linear Regresson  Suppose is a scalar variable. That is there is only one feature. We defer the discussion of multifeature problems to another section. For scalar , a linear predictor of will have only the intercept (the bias) and the slope . The task is to find values of and that minimizes the squared error. Let us include a factor for canceling out a factor that results when you take derivatives. Assuming the the dataset is representative of the entire , we find approximation of the by setting and repacing the integration by the sum. Let's call this expectation value of by  Now, to find the optimal function , here the optimal parameters and , we just have to take partials with each and set to zero. These result in two linear equations in and which are easily solved. We denote the solutions by and . You will find that the following sums help in doing the algebra. After algebra that you should do out, you will find the following expressions for the optimal parameters.     shows a simple linear regression on a dataset consisting of datapoints generated by adding noise to a linear deterministic function by where Here, we have chosen known values of and to generate the data and want to know how well the simple linear regression produces estimates of these parameters.  The simple linear regression using datapoints gave the predicting funtion to be with Compare them to the true values: From the data, we appear to get a better estimate for the slope than that of the intercept.  Note that since the dataset, coming from a random sampling process, is a random variable. Therefore, the estimates and deduced from using are also random variables with their own distributions.   The estimates we gave above are just estimates of their mean values. And, of course, you can do an entire analysis of the significance level of these estimates. While important in statistics books, they will be largely ignored in the Machine Learning work. We take the view that the estimates of the parameters give us the best we can do.   Illustration of a simple linear regression.   The image shows that linear fit to a scatter data.    The figure was generated by the following Python code.  import numpy as np import matplotlib.pyplot as plt # Sample dataset np.random.seed(42) # for reproducibility X = np.linspace(0, 10, 100) Y = 1.0+ 2.0 * X + np.random.normal(0, 1, 100) # Y = 2X + 1 + noise # Calculate means x_bar = np.mean(X) y_bar = np.mean(Y) # Calculate w_1 (slope) numerator = np.sum((X - x_bar) * (Y - y_bar)) denominator = np.sum((X - x_bar) ** 2) w_1 = numerator \/ denominator # Calculate w_0 (intercept) w_0 = y_bar - w_1 * x_bar # Print results print(f\"Estimated w_0 (intercept): {w_0:.2f}\") print(f\"Estimated w_1 (slope): {w_1:.2f}\") # Plot the data and regression line plt.scatter(X, Y, facecolors='none', edgecolors='blue', color='blue', label='Data points') plt.plot(X, w_0 + w_1 * X, color='red', label='Regression line') plt.xlabel('X') plt.ylabel('Y') plt.title('Simple Linear Regression Example') plt.legend() plt.grid(True) plt.show()    "
},
{
  "id": "sec-Regression-2-1",
  "level": "2",
  "url": "sec-Regression.html#sec-Regression-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "regression "
...
{
  "id": "sec-Regression-2-2",
  "level": "2",
  "url": "sec-Regression.html#sec-Regression-2-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Squared Error: Absolute Error: "
},
{
  "id": "sec-Regression-2-3",
  "level": "2",
  "url": "sec-Regression.html#sec-Regression-2-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "parameters "
},
{
  ...
  "level": "2",
  "url": "sec-Regression.html#subsec-The-Simple-Linear-Regresson-3",
  "type": "Example",
  "number": "2.2.1",
  "title": "",
...
},
{
  ...
  "body": " Evaluating Predictions   So far we have talked about learning parameters for models of the conditional probability in various types of problems using a dataset of values called the training dataset . We call the with optimized parameters is called a trained model .  To evaluate the quality of a trained model we need a fresh dataset. In order not to introduce bias in the evalaution of a trained model, we cannot use the same data that was used to train the model in the first place. This pristine set-aside-for-test dataset is called the test dataset . Often, the strategy is to split all the data we have into two subsets for training and testing purposes. We use the test subset solely for testing. After testing with it, we cannot use the knowledge learned to go back an tweak the model to make it perform better on the test. That will make the test set part of the learning process, which we are trying to avoid in order for us to know how the trained model will do on new data later on.  We will use the following notation for the training and test subsets:   The error formula used for training the model may or may not fully align with the end objective of the project. The error formula is usually used for estimating the parameters of the model, which requires taking derivatives of the error function with respect to the parameters. Easy differentiability is not a requirement in the evaluation process. For instance, in a binary classification model, we may just want to know percentage of the times the model predicts the correct label - this will be just be a sum of Kronecker delta symbols, which are not differentiable.  The foremost requirement of evaluation of a model is how it performs on the business requirement of the use of the model. In general, we use a scoring function that closely aligns with the objective of the project. Since these metrics depend on the type of questions we want to answer, it is best to describe them with examples.  First, we will look at a regression task. Then, we will go over a binary classification task. It turns out that binary classification has several important ways for evaluating models and we will try to cover the most common ones below.    Evaluating Regression Tasks  In regression tasks, the trained model is usually a probability density function , where are the estimated or trained parameters. Suppose, we want to predict the best given an , what should be the predictor function and how should we evaluate it? The answer depends on whether is a skewed distribution or not.   Non-skewed Distribution   Suppose, is not skewed. Then, the best prediction for is the mean value of . Therefore, the predictor function will be where I have included the parameter indicator so that we are aware that the predictor function will depend upon the parameters of the model for which are estimated\/trained using the trainign dataset .  To evaluate the quality of this predictor, the average value of the Mean Squared Error is the score function that also minimizes the training error when used during the training f the model. The formula for squared error is Evaluating its average over the test set gives us the mean squared error. That is, mean squared error is empirical expectation of the squared error (a random variable) in the distribution represented by the test dataset, with each datapoint weighing equally. When we use this same formula during the training, we minimze the following error function. where 's are to be adjusted during the training. You should note different uses of the squared error during training and testing for this task.   Non-skewed Distribution   If is skewed or if you suspect that the training data has outliers that have influenced the estimation process, then, a better prediction of best  will be the median of the distribution. The score function in that case would be the expectation of the absolute difference between the predictor and the true. This score function is called mean absolute error (MAE)      Evaluating Binary Classification   In a binary classification, the task is to predict whether a given belongs to one of the two classes, (i.e., True\/Success) or (i.e., False\/Failure). In the logistic regression algorithm, we have modeled the conditional probability mass function , i.e., probability of outcome being for a given , as a sigmoid, whose parameters were optimized by using the training data .  For brevity, let us denote the trained probability by This will be our predictor . AFTER the training is done, we use the predictor with a threshold  for decision . Note that depends upon and the training dataset through the predictor . In our examples in previous sections, we have used the threshold but that may not be optimal for all problems. We will discuss below a systematic way of choosing appropriate threshold for a problem. For now, suppose, we have a rule for converting trained model into predictions on the outcomes and we want to evaluate the quality of resulting predictions.    Metrics of BinaryClassification   Accuracy   To evaluate the trained model, a simple measure is the percentage of the times we got the prediction right, whether it was for or for . This measure is called accuracy of the model. The score function for this measure will just be a Kronecker delta of the predictor and the corresponding . As a random variable, accuracy will by The accuracy of the model over the test set will be the expectation value of this random variable on the test dataset with each datapoint counting equally.    Confusion Matrix   For many problems, accuracy turns out to be good enough for evaluating the quality of predictions. Accuracy, however, misses some details that you might care about. For instance, it lumps together being right on both and cases and you might care more about being right on, say the outcome than on the outcome . In these cases, you might want to look more closely at when you are wrong and when you are right.  Just as there are two events for which you can be right, i.e., when you predict and data (i.e., true) value is also , and when you predict when the data value is also , there are two ways of being wrong: you predict but the data value is and you predict but the data value is . Suppose, we refer to as positive and as negative , then we call these various types of outcomes by different names.    True Positive (TP) : You predict and data (i.e., true) value is also .   True Negative (TN) : You predict and data (i.e., true) value is also .   False Positive (FP) : You predict but the data (i.e., true) value is .   False Negative (FN) : You predict but the data (i.e., true) value is .   Suppose, there are points in the test dataset and you find , , , and . It is a common practice to present these numbers in a matrix, aptly called the confusion matrix .          Actual  Positive  Negative  Totals    Predicted    Predicted    Positive  TP  FP     Negative  FN  TN           Totals Actuals                Actual  Positive  Negative  Totals    Predicted    Predicted    Positive       Negative             Totals Actuals        From the confusion matrix, you can further deduce important metrics of the performance of a model. For instance, we may want to know, What percentage of the actual were we able to predict to be ? It's called recall or True Positive Rate (TPR) . All the positives in the real data are now be in the TP or FN since FN are actually positives which got incorrectly predicted to be negatives. Sometimes, it's more important to know what proportions of all my positive predictions was actuall prositive in the test dataset (i.e., reality). This measure is called precision . There is also a False Positive Rate (FPR) . This measures mistakes in predictions made by the model when looking at the negatives in the test data. It is calculated by dividing the FP (incorrect predictions on negatives in the test data) by the number of datapoints that were actually negatives. The accuracy that we have discussed before will be It would be a good exercise for the student to find the values of these quantities from the table of values given above.   Score:   There is always a tradeoff between precision and recall. For instance,   A cautious classifier will have a high threshold for deciding on positives, which will lead to less False Positives but high False Negatives. That will mean it will give high precision and low recall, as you can verify from the denominators of precision and recall defining equations.    On the other hand, an aggressive classifier will try to label positives more liberally, leading to high False Positives and low False Negatives, which will be a high recall, low precision case.   Sometimes, we want to opt for a balance between these two extremes. That's where we use score. The -score is defined by the harmonic mean of precision and recall: where factor makes sure that if precision and recall are equal, the score will be equal to them also. Expressed in terms of the confusion matrix entries:     Intuitively, F1-score is high only when both precision and recall are high. If either is low, the harmonic mean ensures the F1-score reflects that imbalance. It is particularly useful when we care about both avoiding false positives (FP) and false negatives (FN) equally.   Using Cost Matrix for Evaluation    One way to implement importance of different types of mistakes in predictions is to associate different costs to different outcomes. This is usually displayed in a matrix format. This matrix is called cost matrix , , whose elements will have the following meaning. For, instance, for a binary classificaiton, and . The costs for the correct predictions would, of course, be zero. Thus,               Actual  Positive  Negative      Predicted       Positive       Negative                      The costs incurred when evaluating the performance on the test data can be then weighed accordingly. Thus, in case of the security identification, we might impose a heavy cost of False positive and low for False Negative. How heavy should the relative ratios be? That depends on the requirements of the system. For instance, you could demand a -fold cost when testing.      Evaluating Model Performance Across Thresholds  Metrics like accuracy or F1-score depend on a single classification threshold. Varying this threshold changes true positives (TP), false negatives (FN), false positives (FP), and true negatives (TN). To compare models or choose an optimal threshold, we use two curves: the Receiver Operating Characteristic (ROC) curve and the Precision-Recall (PR) curve. These are similar for balanced datasets but differ significantly for imbalanced ones, where PR curves are more informative.   ROC Curve and AUC-ROC   The ROC curve plots the True Positive Rate (TPR, or Recall) against the False Positive Rate (FPR) across all thresholds: It shows how well a model separates positives from negatives. The Area Under the ROC Curve (AUC-ROC) summarizes this: AUC-ROC = 1 for a perfect model, 0.5 for a random one. AUC-ROC measures the probability that a positive instance is ranked higher than a negative one.   Precision-Recall Curve and AUC-PR   The PR curve plots Precision against Recall: It focuses on positive class performance, making it ideal for imbalanced datasets where positives are rare. The Area Under the PR Curve (AUC-PR) , or Average Precision, quantifies this performance: AUC-PR = 1 for a perfect model, lower for poor positive-class predictions.   Uses of AUC-ROC vs. AUC-PR   AUC-ROC is best for balanced datasets or when false positives and negatives have similar costs, as it evaluates overall discrimination. AUC-PR is better for imbalanced datasets (e.g., fraud detection, rare disease diagnosis), as it emphasizes precision and recall for the positive class, avoiding the misleading optimism of ROC when negatives dominate.  Following are key differences in the ROC and PR curves.   Receiver-Operating Characterisitcs (ROC) Curve    Aspect  ROC Curve    X-axis  FPR = FP \/ (FP + TN)    Y-axis  TPR = Recall = TP \/ (TP + FN)    Focus  Discrimination between positives and negatives overall    Effect of class imbalance  Low; ROC may look optimistic in rare positives    Use case  General model comparison; balanced datasets      Precision-Recall (PR) Curve    Aspect  PR Curve    X-axis  Recall = TP \/ (TP + FN)    Y-axis  Precision = TP \/ (TP + FP)    Focus  Performance on positive class, correctness of positive predictions    Effect of class imbalance  High; PR shows poor precision when positives are rare    Use case  Imbalanced datasets or when positive-class performance is critical      Selecting a Preferred Model   To choose between models, compare their AUC-ROC (for balanced data) or AUC-PR (for imbalanced data). Higher AUC indicates better performance. For ROC, the curve closest to the top-left corner is best; for PR, it’s the top-right. If false positives are costly (e.g., spam detection), pick the model with higher precision at the desired recall level from the PR curve.   Example: Balanced vs. Imbalanced Datasets   Consider a dataset with 1000 samples:  Balanced: 500 positives, 500 negatives.  Imbalanced: 100 positives, 900 negatives.  In balanced datasets, ROC and PR curves look similar. In imbalanced datasets, ROC may seem optimistic due to low FPR, while PR reveals low precision for rare positives.   Python Example: Shading AUC for Two Models   The following code compares two models on a balanced dataset: a \"good\" logistic regression model and a \"poor\" one trained on noisy features. It plots ROC and PR curves, shading the AUC to highlight performance differences. Run this code to generate the figures shown below.   ROC and Precision-Recall curves with shaded AUC comparing a good and poor model on a balanced dataset. The dots on the curves show the optimal threshold for the models; Youden's index (max TPR - FPR) for optimal for ROC and max F1 score for optimal PR were chosen by Grok.    # The code produced by Grok import numpy as np from sklearn.datasets import make_classification from sklearn.model_selection import train_test_split from sklearn.linear_model import LogisticRegression from sklearn.metrics import roc_curve, precision_recall_curve, roc_auc_score, average_precision_score import matplotlib.pyplot as plt # Good model: Easy dataset, balanced, many informative features X_good, y_good = make_classification(n_samples=5000, n_features=20, n_informative=18, n_redundant=0, n_clusters_per_class=1, weights=[0.5, 0.5], flip_y=0.0, random_state=42) X_train_good, X_test_good, y_train_good, y_test_good = train_test_split(X_good, y_good, test_size=0.3, random_state=42) good_model = LogisticRegression().fit(X_train_good, y_train_good) y_prob_good = good_model.predict_proba(X_test_good)[:, 1] # Poor model: Hard dataset, imbalanced, few informative, much noise, label noise X_poor, y_poor = make_classification(n_samples=1000, n_features=20, n_informative=2, n_redundant=10, n_clusters_per_class=2, weights=[0.95, 0.05], flip_y=0.2, random_state=0) X_train_poor, X_test_poor, y_train_poor, y_test_poor = train_test_split(X_poor, y_poor, test_size=0.3, random_state=42) poor_model = LogisticRegression().fit(X_train_poor, y_train_poor) y_prob_poor = poor_model.predict_proba(X_test_poor)[:, 1] # ROC curves fpr_g, tpr_g, thresh_g = roc_curve(y_test_good, y_prob_good) auc_roc_g = roc_auc_score(y_test_good, y_prob_good) # Optimal for ROC: Youden's index (max TPR - FPR) youden_idx_g = np.argmax(tpr_g - fpr_g) opt_fpr_g = fpr_g[youden_idx_g] opt_tpr_g = tpr_g[youden_idx_g] # PR prec_g, rec_g, thresh_pr_g = precision_recall_curve(y_test_good, y_prob_good) auc_pr_g = average_precision_score(y_test_good, y_prob_good) # Optimal for PR: max F1 score f1_g = 2 * prec_g * rec_g \/ (prec_g + rec_g + 1e-10) f1_idx_g = np.argmax(f1_g[:-1]) # Exclude last point as it's invalid opt_rec_g = rec_g[f1_idx_g] opt_prec_g = prec_g[f1_idx_g] # Similar for poor fpr_p, tpr_p, thresh_p = roc_curve(y_test_poor, y_prob_poor) auc_roc_p = roc_auc_score(y_test_poor, y_prob_poor) youden_idx_p = np.argmax(tpr_p - fpr_p) opt_fpr_p = fpr_p[youden_idx_p] opt_tpr_p = tpr_p[youden_idx_p] prec_p, rec_p, thresh_pr_p = precision_recall_curve(y_test_poor, y_prob_poor) auc_pr_p = average_precision_score(y_test_poor, y_prob_poor) f1_p = 2 * prec_p * rec_p \/ (prec_p + rec_p + 1e-10) f1_idx_p = np.argmax(f1_p[:-1]) opt_rec_p = rec_p[f1_idx_p] opt_prec_p = prec_p[f1_idx_p] # Plot plt.figure(figsize=(12, 6)) plt.subplot(1, 2, 1) plt.plot(fpr_g, tpr_g, label=f'Good (AUC = {auc_roc_g:.2f})', color='blue') plt.fill_between(fpr_g, tpr_g, color='blue', alpha=0.2) plt.plot(fpr_p, tpr_p, label=f'Poor (AUC = {auc_roc_p:.2f})', color='orange') plt.fill_between(fpr_p, tpr_p, color='orange', alpha=0.2) plt.plot([0, 1], [0, 1], 'k--') plt.scatter(opt_fpr_g, opt_tpr_g, marker='o', color='blue', label='Good Opt (Youden)') plt.scatter(opt_fpr_p, opt_tpr_p, marker='o', color='orange', label='Poor Opt (Youden)') plt.xlabel('False Positive Rate') plt.ylabel('True Positive Rate') plt.title('ROC Curves') plt.legend() plt.subplot(1, 2, 2) plt.plot(rec_g, prec_g, label=f'Good (AP = {auc_pr_g:.2f})', color='blue') plt.fill_between(rec_g, prec_g, color='blue', alpha=0.2) plt.plot(rec_p, prec_p, label=f'Poor (AP = {auc_pr_p:.2f})', color='orange') plt.fill_between(rec_p, prec_p, color='orange', alpha=0.2) plt.scatter(opt_rec_g, opt_prec_g, marker='o', color='blue', label='Good Opt (F1 max)') plt.scatter(opt_rec_p, opt_prec_p, marker='o', color='orange', label='Poor Opt (F1 max)') plt.xlabel('Recall') plt.ylabel('Precision') plt.title('PR Curves') plt.legend() plt.tight_layout() plt.show()    "
},
{
...
{
  "id": "sec-Evaluating-Predictions-2-5",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#sec-Evaluating-Predictions-2-5",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsec-Evaluating-Binary-Classification-2-2",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#subsec-Evaluating-Binary-Classification-2-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "predictor threshold decision "
},
{
  "id": "subsubsec-Metrics-of-Binary-Classification-3",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#subsubsec-Metrics-of-Binary-Classification-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsubsec-Metrics-of-Binary-Classification-6",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#subsubsec-Metrics-of-Binary-Classification-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsubsec-Metrics-of-Binary-Classification-8",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#subsubsec-Metrics-of-Binary-Classification-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsubsec-Metrics-of-Binary-Classification-14",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#subsubsec-Metrics-of-Binary-Classification-14",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "cost matrix "
...
{
  "id": "subsubsec-ROC-PR-Curves-AUC-2-4",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#subsubsec-ROC-PR-Curves-AUC-2-4",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "ROC curve Area Under the ROC Curve (AUC-ROC) "
},
{
  "id": "subsubsec-ROC-PR-Curves-AUC-2-6",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#subsubsec-ROC-PR-Curves-AUC-2-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "PR curve Area Under the PR Curve (AUC-PR) "
},
{
  "id": "subsubsec-ROC-PR-Curves-AUC-2-10",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#subsubsec-ROC-PR-Curves-AUC-2-10",
  "type": "Table",
  "number": "2.6.1",
  "title": "Receiver-Operating Characterisitcs (ROC) Curve",
  "body": " Receiver-Operating Characterisitcs (ROC) Curve    Aspect  ROC Curve    X-axis  FPR = FP \/ (FP + TN)    Y-axis  TPR = Recall = TP \/ (TP + FN)    Focus  Discrimination between positives and negatives overall    Effect of class imbalance  Low; ROC may look optimistic in rare positives    Use case  General model comparison; balanced datasets    "
},
{
  "id": "subsubsec-ROC-PR-Curves-AUC-2-11",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#subsubsec-ROC-PR-Curves-AUC-2-11",
  "type": "Table",
  "number": "2.6.2",
  "title": "Precision-Recall (PR) Curve",
  "body": " Precision-Recall (PR) Curve    Aspect  PR Curve    X-axis  Recall = TP \/ (TP + FN)    Y-axis  Precision = TP \/ (TP + FP)    Focus  Performance on positive class, correctness of positive predictions    Effect of class imbalance  High; PR shows poor precision when positives are rare    Use case  Imbalanced datasets or when positive-class performance is critical    "
},
{
  "id": "fig-roc-pr-auc-shaded",
  "level": "2",
  "url": "sec-Evaluating-Predictions.html#fig-roc-pr-auc-shaded",
  "type": "Figure",
  "number": "2.6.3",
  "title": "",
  "body": " ROC and Precision-Recall curves with shaded AUC comparing a good and poor model on a balanced dataset. The dots on the curves show the optimal threshold for the models; Youden's index (max TPR - FPR) for optimal for ROC and max F1 score for optimal PR were chosen by Grok.   "
},
{
  ...
  "body": " Bayesian Methods   In supervised learning, we often model the conditional probability . This naturally appears in Bayes's theorem as the posterior probability . Recall the identity: Equivalently, Bayes's theorem can be written as When we use Bayes's theorem to model conditional probabilities and guide decision-making, the resulting approaches are called Bayesian methods .  For comparing possible outcomes given the same , the denominator cancels out, so we only need relative probabilities: Let us call the right side un-normalized probabilities . Then, to make decision of which outcome is more likely, we just compare their unnormalized probabilities rather than the actual probabilities since the two woudl give the same answer. Therefore, we choose over by Thus the unnormalized probabilites are sufficient to decide which outcome of is more probable for a given without needing to know the full distribution . All we need are:   The conditional distributions , and    The prior probabilities , i.e., the marginal probabilities of each outcome of .     If is a discrete variable with classes, then, you would just use the function . Basically, arg max goes over the list of values generated on the right side for various values of , i.e., for , and then outputs the index of , which corresponds to the largest value in the list. If you have ties, then it outputs the first in the list. You can also have it printout all the items in the tie and output one randomly from that sublist.  Bayes's method is used not only for making predictions of the labels, but also to learn parameters of a model. Suppose we have a dataset , where I am writing that includes both and . We assume that the datapoints are actually samples from some unknown distribution (mass function or density). We model this distribution by and would like to estimate the parameters using the data. So, we use Bayes's and cast the problem of improving from evolution of our knowledge about from the prior to the posterior . where we can think of , the marginal for the alone, as just a normalization of the unnormalized probability density in the numerator on the right, i.e., Now, each datapoint can be used to update the posterior. We do not go over this approach further here and will return at this issue later.  In this section, we will next discuss a very commonly used approximate method known as the Naive Bayes method.    Naive Bayes Algorithm  The Naive Bayes algorithm applies Bayes’ theorem under a simplifying assumption: the features of are conditionally independent given the class label . Formally, This assumption is rarely true in practice, but it makes the computation of tractable, and the resulting classifier often performs surprisingly well.  Substituting this factorization into Bayes’ theorem, we obtain A Naive Bayes classifier assigns the label    Suppose we want to classify whether an email is spam or ham based on two binary features: “contains the word ‘free’” and “contains the word ‘money’.” From training data we estimate: If an email contains both words ( ), then Since , the classifier predicts spam .    Suppose we want to classify whether an email is spam or ham based on two binary features:  contains the word 'deal' and  contains the word 'win'. From training data we estimate: Now consider three scenarios:   If the email contains only “deal” ( ): The classifier predicts ham .    If the email contains only “win” ( ): Again the classifier predicts ham .    If the email contains both words ( ): Now the classifier predicts spam .       In this example, I include a complete Python program, generated with the assistance of OpenAI's ChatGPT (GPT-5), September 2025, that uses the Naive Bayes classifier, called MultinomialNB(), from scikit-learn. I have tested it on Google Colab and locally on my Mac and it works correctly. Scikit-learn library saves a lot of hassle of programming from scratch, which we have practiced on simpler problems before to get a sense of the math steps. Later on, we will use other libraries such as TensorFlow and PyTorch.  Notice the preprocessing step of the data handling. A lot of people get stuck at that step since it is highly problem dependent. Now a days, you can just ask one of the AI models to help you out on that step and more likely than not you will get a reasonably good way to handle your data as long as you give it a complete prompt on what you have and what you want it to be.   The program loads a dataset that's freely available online (the SMS Spam Collection dataset hosted by UCI) by downloading the dataset (4,827 ham, 747 spam). Uses a bag-of-words (CountVectorizer) to turn text into features.    Splits into train\/test sets. That sets a part of the data for testing purposes ONLY.    Trains the Naive Bayes model MultinomialNB().    Evaluates it using a confusion matrix.    Prints precision, recall, F1-score.     When you run the program, you would get outputs like the following.  Vocabulary size: 7441 Accuracy: 0.9883408071748879  Confusion Matrix:   The confuction matrix. Note: here the actuals are left to right and predicted are from top to bottom.   The Confusion Matrix.    Classification Report:     precision  recall  f1-score  support    ham  0.99  0.99  0.99  966    spam  0.97  0.95  0.96  149    accuracy    0.99  1115    macro avg  0.98  0.97  0.97  1115    weighted avg  0.99  0.99  0.99  1115    New message predictions (0=ham, 1=spam):  Message: Free offer! Win a $1000 gift card now!... –> spam  Message: Hey, let's meet tomorrow at 5pm.... –> ham  # Naive Bayes spam classifier with confusion matrix import matplotlib.pyplot as plt import pandas as pd import seaborn as sns from sklearn.feature_extraction.text import CountVectorizer from sklearn.metrics import classification_report, confusion_matrix from sklearn.model_selection import train_test_split from sklearn.naive_bayes import MultinomialNB # ----------------------- # 1. Load dataset # ----------------------- # Use a GitHub-hosted copy of the SMS Spam Collection dataset url = \"https:\/\/raw.githubusercontent.com\/justmarkham\/DAT8\/master\/data\/sms.tsv\" df = pd.read_csv(url, sep=\"\\t\", names=[\"label\", \"message\"]) print(\"Dataset shape:\", df.shape) print(df.head()) # ----------------------- # 2. Preprocess # ----------------------- df[\"label\"] = df[\"label\"].map({\"ham\": 0, \"spam\": 1}) X_train, X_test, y_train, y_test = train_test_split( df[\"message\"], df[\"label\"], test_size=0.2, random_state=42 ) # Convert text to bag-of-words vectorizer = CountVectorizer(stop_words=\"english\") X_train_vec = vectorizer.fit_transform(X_train) X_test_vec = vectorizer.transform(X_test) # ----------------------- # 3. Train model # ----------------------- model = MultinomialNB() model.fit(X_train_vec, y_train) # ----------------------- # 4. Evaluate # ----------------------- y_pred = model.predict(X_test_vec) cm = confusion_matrix(y_test, y_pred) plt.figure(figsize=(5, 4)) sns.heatmap( cm, annot=True, fmt=\"d\", cmap=\"Blues\", xticklabels=[\"Ham\", \"Spam\"], yticklabels=[\"Ham\", \"Spam\"], ) plt.xlabel(\"Predicted\") plt.ylabel(\"Actual\") plt.title(\"Confusion Matrix\") plt.tight_layout() plt.show() print(\"\\nClassification Report:\\n\") print(classification_report(y_test, y_pred, target_names=[\"Ham\", \"Spam\"]))    "
},
{
...
{
  "id": "subsec-Naive-Bayes-4",
  "level": "2",
  "url": "sec-Bayesian-Methods.html#subsec-Naive-Bayes-4",
  "type": "Example",
  "number": "2.7.1",
  "title": "",
  "body": " Suppose we want to classify whether an email is spam or ham based on two binary features: “contains the word ‘free’” and “contains the word ‘money’.” From training data we estimate: If an email contains both words ( ), then Since , the classifier predicts spam .  "
},
{
  "id": "subsec-Naive-Bayes-5",
  "level": "2",
  "url": "sec-Bayesian-Methods.html#subsec-Naive-Bayes-5",
  "type": "Example",
  "number": "2.7.2",
  "title": "",
  "body": " Suppose we want to classify whether an email is spam or ham based on two binary features:  contains the word 'deal' and  contains the word 'win'. From training data we estimate: Now consider three scenarios:   If the email contains only “deal” ( ): The classifier predicts ham .    If the email contains only “win” ( ): Again the classifier predicts ham .    If the email contains both words ( ): Now the classifier predicts spam .     "
},
{
  ...
  "number": "2.7.3",
  "title": "",
  "body": " In this example, I include a complete Python program, generated with the assistance of OpenAI's ChatGPT (GPT-5), September 2025, that uses the Naive Bayes classifier, called MultinomialNB(), from scikit-learn. I have tested it on Google Colab and locally on my Mac and it works correctly. Scikit-learn library saves a lot of hassle of programming from scratch, which we have practiced on simpler problems before to get a sense of the math steps. Later on, we will use other libraries such as TensorFlow and PyTorch.  Notice the preprocessing step of the data handling. A lot of people get stuck at that step since it is highly problem dependent. Now a days, you can just ask one of the AI models to help you out on that step and more likely than not you will get a reasonably good way to handle your data as long as you give it a complete prompt on what you have and what you want it to be.   The program loads a dataset that's freely available online (the SMS Spam Collection dataset hosted by UCI) by downloading the dataset (4,827 ham, 747 spam). Uses a bag-of-words (CountVectorizer) to turn text into features.    Splits into train\/test sets. That sets a part of the data for testing purposes ONLY.    Trains the Naive Bayes model MultinomialNB().    Evaluates it using a confusion matrix.    Prints precision, recall, F1-score.     When you run the program, you would get outputs like the following.  Vocabulary size: 7441 Accuracy: 0.9883408071748879  Confusion Matrix:   The confuction matrix. Note: here the actuals are left to right and predicted are from top to bottom.   The Confusion Matrix.    Classification Report:     precision  recall  f1-score  support    ham  0.99  0.99  0.99  966    spam  0.97  0.95  0.96  149    accuracy    0.99  1115    macro avg  0.98  0.97  0.97  1115    weighted avg  0.99  0.99  0.99  1115    New message predictions (0=ham, 1=spam):  Message: Free offer! Win a $1000 gift card now!... –> spam  Message: Hey, let's meet tomorrow at 5pm.... –> ham  # Naive Bayes spam classifier with confusion matrix import matplotlib.pyplot as plt import pandas as pd import seaborn as sns from sklearn.feature_extraction.text import CountVectorizer from sklearn.metrics import classification_report, confusion_matrix from sklearn.model_selection import train_test_split from sklearn.naive_bayes import MultinomialNB # ----------------------- # 1. Load dataset # ----------------------- # Use a GitHub-hosted copy of the SMS Spam Collection dataset url = \"https:\/\/raw.githubusercontent.com\/justmarkham\/DAT8\/master\/data\/sms.tsv\" df = pd.read_csv(url, sep=\"\\t\", names=[\"label\", \"message\"]) print(\"Dataset shape:\", df.shape) print(df.head()) # ----------------------- # 2. Preprocess # ----------------------- df[\"label\"] = df[\"label\"].map({\"ham\": 0, \"spam\": 1}) X_train, X_test, y_train, y_test = train_test_split( df[\"message\"], df[\"label\"], test_size=0.2, random_state=42 ) # Convert text to bag-of-words vectorizer = CountVectorizer(stop_words=\"english\") X_train_vec = vectorizer.fit_transform(X_train) X_test_vec = vectorizer.transform(X_test) # ----------------------- # 3. Train model # ----------------------- model = MultinomialNB() model.fit(X_train_vec, y_train) # ----------------------- # 4. Evaluate # ----------------------- y_pred = model.predict(X_test_vec) cm = confusion_matrix(y_test, y_pred) plt.figure(figsize=(5, 4)) sns.heatmap( cm, annot=True, fmt=\"d\", cmap=\"Blues\", xticklabels=[\"Ham\", \"Spam\"], yticklabels=[\"Ham\", \"Spam\"], ) plt.xlabel(\"Predicted\") plt.ylabel(\"Actual\") plt.title(\"Confusion Matrix\") plt.tight_layout() plt.show() print(\"\\nClassification Report:\\n\") print(classification_report(y_test, y_pred, target_names=[\"Ham\", \"Spam\"]))  "
},
{
  ...
  "body": " Model Selection   Suppose you have trained more than one model using the same training data and would like to choose the best among them. That is, you have a number of trained\/estimated predictors, and we want to pick best among them. This task is called model selection . It is done using a part of the training dataset that is called validation data as we explain below.    Validation Data  Since, in the model selection, we cannot yet use the set-aside test dataset to evaluate the final model, you need to set aside a part of your training data itself for the model selection step. This dataset is called validation data . Unlike the set-aside dataset, which is used only on the final model, you can use the validation data as many times as needed during the training.  Peviously, we used datapoints as training and datapints as set-aside test dataset. Now, we will have three datasets training examples, validation examples and testing examples. The proportion of total data that should be placed in the three bins varies from project to project and availability of data. If we have plenty of data to work with, a common breakdown is shown in .   If there is plentiful data, one can use (or ) train\/validation\/test split. In other cases, other strategies such as cross-validation are more common.   (for accessibility)    Often we do not have the luxury of plenty of data. Then, we have to be creative how to split all the data that we have to work with into three categories. First, you must set aside a reasonable amount for testing, say , for the final model evaluation. Then, you treat the entirety of the rest of the data as both training and validation data, but you do not use the same datapoints in both in any one training cycle. Let's call this set a combined training dataset . Now, we can choose from several strategies to implement the cross-validation.      Leave p out cross validation (LOOCV). You do multiple runs through the combined training data, and in each cycle, you leave different examples out for validation and use the rest for training. That is, you never use the examples you used for training as validation. In an extereme case of this strategy, there is Leave One Out policy, when .    A more common strategy is K-fold cross-validation . In each training cycle, you split the combined training data into equally sized folds . You use of these fold for training and one fold for validation as illustrated in . We will go over K-fold cross validation in more detail below in another section.  In a stratified K-fold cross-validation, you attempt to ensure that each fold has the same proportions of each class. The stratified K-fold is specially important if you have an imbalanced data, which happens when one class or a number of classes are either over-represented or underrepresented in the dataset.      Illustration of the operations of a K-fold cross-validation for .   (for accessibility)          Generalization Error  After training any model, we want the model to perform well on new data. The error on a new data that was not used in the training of the model, is called generalizaton error or out-of-sample error . This error gives us the clearest picture of how the trained model will perform in the real world.  Let us illustrate this concept in the context of a regression task of a quantitative or interval target variable and a -dimensional feature vector with error being the square error. With the trained predictor , the squared error (a random variable) will be If you evaluate it's expectation in the training dataset , we call the result the training error or in-sample error . This is of course of not much use since this error is on the same datapoints that were used to produce the trained model in the first place. It is mostly used for diagnositc purposes since this error shoudl go down with each training of the model as the model learns to fit better and better to the training dataset.  Evaluating the error of Eq. on a new data , we will get The expectation of the error on a validattion dataset will be This is an out-of-sample or generalization error. We use validation error to decide among competing models. Similarly, on a set-aside test dataset, This is also an out-of-sample or generalization error. We use this test error to evaluate the final model.    Bias and Variance of Model in Generalization Error  To understand the various sources of generalization errors let us look more closely at the expected error in a single input vector . Here the trained prediction model depends on the training dataset . Now, notice that the sample is itself a random sample from the unknown joint distribution . This results in a distribution of itself. We just need mean and variance of for the purpose of assessing the squared error. Let us define them by the following expectation values with respect to distribution. We can also obtain the expectation by averaging over all possible sample datasets from the -space. We usually write this averaging process as follows. In Eq. , we now are left with handling , which we can model by a random Gaussian noise added to the true functional relation as follows. Now, the expectation with respect to for a given , will be just expectation with respect to the noise variable . That is, for the mean and variance of you would get Note that is a fixed number and therefore, it's variance is zero. From this and that , we can easily surmize that We will use this relation below.  Now, we have the pieces to find the expectation of the error variable in Eq. for a given input . In the last step, the middle term is zero since factors out and its expectation is zero. Now, we introduce expectation of , to be denoted by , inside the last term as a plus and a minus term. Here Now, we group the last term into two groups: When we expand , the expectation of the cross-term truns out to be zero as the following series of steps demostrates. The expectation of is just itself, since the random variable inside it already averaged over. Therefore, only the expectations of remain in the abstract form giving us the final expression for the expectation of the generalization error on a single datapoint, . The error has three terms: the first term is called the irreducible error - it is completely model-independent and is inherent to the statistical nature of the variables themselves. The second term is the square of how different the prediction of the mean model is from the true value at the datapoint under consideration. This is called bias of the model. The contribution of this term to the error appears as it's square and therefore it is called Bias . The last term in the error is the variance of the model in its distribution. Basically, model will change with different training samples, resulting in a collection of models, and this error comes from how variable various models would be from the mean model, which is designated here by .  Summarizing, This result is called Bias-Variance Decomposition of the expected error. Altough, we have arrived at it by examining squared error for a regression task, one can show that the decomposition is general and works out for other tasks as well. We will see below that this decomposition is important in understanding how to handle complexity in models.  When creating a machine learning model, the challenge is to design it so it learns the real patterns without making too many mistakes. We see here that the two main reasons for error are bias and variance. High bias and low variance comes from using an overly simple model that misses the mark in a consistent way, i.e., consistently wrong. Variance happens when the model reacts too strongly to the training data, leading to big swings in its predictions.  The trick is to manage both at the same time. Too much bias means the model underfits and overlooks useful information. Too much variance means it overfits, confusing random noise for actual patterns. Good models sit somewhere in the middle, which is why strong results in training don't always carry over to new situations.    Bias-Variance Tradeoff in Practice  When creating a machine learning model, the challenge is to design it so it learns the real patterns without making too many mistakes. We saw above that the two main reasons for generalization error are bias and variance. High bias and low variance comes from using an overly simple model that misses the mark in a consistent way. Variance happens when the model reacts too strongly to the training data, leading to big swings in its predictions.  The trick is to manage both at the same time. Too much bias means the model underfits and overlooks useful information as shown in . Too much variance means it overfits , confusing random noise for actual patterns. Good models sit somewhere in the middle, which is why strong results in training don't always carry over to new situations.   Illustration of underfit, right fit, and overfit. The fitting was done by using polynomials of various degrees as shown.       Another way to look at the role of complexity in the bias-variance tradeoff is shown in . As you increase the complexity of the model, the bias will tend to go down, but the variance will increase such that the net effect is that error is minimum for an intermediate complexity.   Illustration of change of bias-square and variance with the complexity of the model.       As we can see in these figures that high-bias models, such as linear regression when the actual data is nonlinear, are usually too simplistic to capture the true pattern in the data. We say that they underfit the data. This leads to high errors on both the training set itself and the validation dataset. On the other side are the high variance models, such as decision trees without pruning and neural networks without regularization, which tend to have too much flexibility and fit even the noise in the training data. This overfitting of data gives a low error on the training set but high error on the validation dataset.   Leaning curves are used to diagnose underfitting and overfitting problems in a model. As shown in , we train the model using multiple training sets of varying sizes and plot training and validation errors versus size of the training set. In the underfitting case, both training and validation errors stay high with the gap between them remaining small, indicating the the model is too simplistic to capture patterns. In the overfitting case, the training error is very low and the validation error stays much higher, i.e., the model memorizes the training data but fails on unseen data.   Learning curves help diagnose underfitting and overfitting problems in a model.        Controlling Bias-Variance Tradeoff by Regularization   In Machine Learning, we want to lean on the side of overfitting and then reducing the overfitting by various techniques, which goes by the name regularization . These techniques range from adding penalty on the trainable parameters, e.g., ridge and lasso regression, and randomly dropping activities in a neuronal netword in a method called dropout.  For instance, in lasso regression, you add a penalty term to the training loss function. Then, the optimization problem becomes Clearly, minimization of the error will include minimizing the penalty term, which would be minimized if the coefficients become smaller. As a result, the larger the value of , the more the length of will shrink.  If the penalty term is sum of the absolute values of the components, then shrinkage leads to some of the components actually becoming zero, i.e., those components of the feature vector get zero influence in predicting the outcome. This is called lasso regression.     K-fold Cross Validation  Cross Validation is perhaps the most used method for comparing and selecting models based on an estimate of the generalization errors by those models. As we have discussed above in the K-fold cross-validation, we use parts to fit (i.e., train) the model and the remaining part as the validation data to test the trained model.  For instance, if you have , we will have folds as shown in . In one cycle, we train on all the data except one of the folds. Suppose, we have picked the fold for validation, then we train on all data in folds , which we can indicate by , folds except .  Suppose we have a model that we want to train by optimizing using the Gradient Descent. Suppose the model also has a hyperparameter which needs to be optimized as well. Hyperparameters are parameters that are not trained by the training loop. That makes the model actually indexed by the hyperparameter, viz., .  We first get the optimized model using the data in for one value of , say . Then, we get the validation error on this model on the current fold. The error can be improved by repeating the training on , generating values of , which can be averaged to get a better estimate of the generalization error.  Now, we repeat the training loop for a different , say with the validation error We can repeat for as many 's as we need. Then, we can use the validation errors, to pick the that gave the least validation error.   In this example we illustrate the use of cross-validation to pick the best model by fitting a noisy sinusoid by polynomilas with unknown best degree of the polynomial. Here, are the coefficients, which will be obtained by a linear regression algorithm and , the degree of polynomial of the model, is the hyperparameter. To find the best value of the hyperparameter we use the generalization error on the validation subset in a 5-fold cross-validation procedure. For error, we will use the square-root of the square error we have talked above by computing mean root mean squared error (RMSE) on the validation subsets of various folds. Note, the are the optimum values of the coefficients obtained by using the training dataset. Only after we have obtained the trained model, we compute the RMSE on the validation dataset. All of these steps can be implemented from scratch or you can use excellent standard libraries such as sklearn and PyTorch.  The program listed below first generates some data with noise. Then, we try to fit the data with polynomial regression models of varying degrees. The function cross_val_score(...) takes that model and performs the following steps for each fold:   Splits the data into training and validation sets (according to the KFold splitter).    Trains (fits) the model on the training set. Since, this model is just a linear regression, the fit is just one step. If we had a neural network, we would train for a number of epochs, i.e., cycles through the trianing subsets.    Evaluates it on the validation set using the chosen metric (in this case, negative MSE).    Repeats this across all folds.    The function then returns the list of validation scores (one per fold).      shows the result for different values of along the abscissa, each of those values correspond to a different model. This plot may produce a clear U-shaped curve as in this figure, but often can be very noisy. Here, low-degree polynomials (deg 1 and 2), have high bias and high error, clear indicators of underfit. The high-degree polynomials have high validation error, which is an indicator of overfit. The intermediate range has the best compromise with the least error.   Illustration of the use of cross-validation to select the model with the least generalization error. The data is fit with polynomials of different degrees by regression.       # This program is based on a program generated by GPT-5. import numpy as np import matplotlib.pyplot as plt from sklearn.preprocessing import PolynomialFeatures from sklearn.linear_model import LinearRegression from sklearn.pipeline import make_pipeline from sklearn.model_selection import cross_val_score, KFold plt.rcParams['axes.labelsize'] = 14 plt.rcParams['xtick.labelsize'] = 12 plt.rcParams['ytick.labelsize'] = 12 plt.rcParams['legend.fontsize'] = 12 # 1. Generate synthetic data np.random.seed(42) X = np.linspace(0, 5, 50).reshape(-1, 1) y = np.sin(X).ravel() + np.random.normal(scale=0.6, size=X.shape[0]) # noisy sine wave # 2. Hyperparameter: polynomial degree degrees = range(1, 11) cv_rmse = [] kf = KFold(n_splits=5, shuffle=True, random_state=42) # 3. Cross-validation for each degree for deg in degrees: model = make_pipeline(PolynomialFeatures(deg), LinearRegression()) scores = cross_val_score(model, X, y, cv=kf, scoring='neg_mean_squared_error') # implements the cross-validation rmse_mean = np.sqrt(-scores.mean()) cv_rmse.append(rmse_mean) print(f\"Degree={deg:2d} -> CV mean RMSE = {rmse_mean:.3f}\") # 4. Find best degree best_deg = degrees[np.argmin(cv_rmse)] best_rmse = min(cv_rmse) print(f\"\\nBest polynomial degree = {best_deg}, CV mean RMSE = {best_rmse:.3f}\") # 5. Plot plt.figure(figsize=(8,5)) plt.plot(degrees, cv_rmse, marker='o', color='blue') plt.axvline(best_deg, color='red', linestyle='--', label=f\"Best polynomial degree = {best_deg}\") plt.xlabel(\"Polynomial Degree\") plt.ylabel(\"Mean CV RMSE\") plt.xticks(degrees) plt.legend(loc=\"upper center\") plt.grid(True, linestyle='--', alpha=0.6) plt.tight_layout() plt.show()    "
},
{
  "id": "sec-Model-Selection-2-1",
  "level": "2",
  "url": "sec-Model-Selection.html#sec-Model-Selection-2-1",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsec-Validation-Data-2",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Validation-Data-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "fig-data_split_proportions",
  "level": "2",
  "url": "sec-Model-Selection.html#fig-data_split_proportions",
  "type": "Figure",
  "number": "2.8.1",
  "title": "",
  "body": " If there is plentiful data, one can use (or ) train\/validation\/test split. In other cases, other strategies such as cross-validation are more common.   (for accessibility)   "
},
{
  "id": "subsec-Validation-Data-6",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Validation-Data-6",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Leave p out cross validation K-fold cross-validation folds stratified K-fold "
},
{
  "id": "fig-kfold_cv",
  "level": "2",
  "url": "sec-Model-Selection.html#fig-kfold_cv",
  "type": "Figure",
  "number": "2.8.2",
  "title": "",
  "body": " Illustration of the operations of a K-fold cross-validation for .   (for accessibility)   "
},
{
  "id": "subsec-Out-of-Sample-Error-2",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Out-of-Sample-Error-2",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "generalizaton error out-of-sample error "
},
{
  "id": "subsec-Out-of-Sample-Error-3",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Out-of-Sample-Error-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "training error in-sample error "
},
{
  "id": "subsec-Sources-of-Generalization-Error-3",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Sources-of-Generalization-Error-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "irreducible error bias Bias variance "
},
{
  "id": "subsec-Bias-Variance-in-Practice-3",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Bias-Variance-in-Practice-3",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "underfits overfits "
},
{
  "id": "fig-bias_variance_tradeoff",
  "level": "2",
  "url": "sec-Model-Selection.html#fig-bias_variance_tradeoff",
  "type": "Figure",
  "number": "2.8.3",
  "title": "",
  "body": " Illustration of underfit, right fit, and overfit. The fitting was done by using polynomials of various degrees as shown.      "
},
{
  "id": "fig-bias_variance_tradeoff_and_complexity",
  "level": "2",
  "url": "sec-Model-Selection.html#fig-bias_variance_tradeoff_and_complexity",
  "type": "Figure",
  "number": "2.8.4",
  "title": "",
  "body": " Illustration of change of bias-square and variance with the complexity of the model.      "
},
{
  "id": "subsec-Bias-Variance-in-Practice-8",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Bias-Variance-in-Practice-8",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "Leaning curves "
...
{
  "id": "fig-learning_curves",
  "level": "2",
  "url": "sec-Model-Selection.html#fig-learning_curves",
  "type": "Figure",
  "number": "2.8.5",
  "title": "",
  "body": " Learning curves help diagnose underfitting and overfitting problems in a model.      "
},
{
  "id": "subsec-Bias-Variance-in-Practice-11",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Bias-Variance-in-Practice-11",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "regularization "
...
{
  "id": "subsec-Bias-Variance-in-Practice-12",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Bias-Variance-in-Practice-12",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "lasso "
...
{
  "id": "subsec-Bias-Variance-in-Practice-13",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-Bias-Variance-in-Practice-13",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
  "body": "lasso "
...
{
  "id": "subsec-K-fold-cross-validation-4",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-K-fold-cross-validation-4",
  "type": "Paragraph (with a defined term)",
  "number": "",
  "title": "",
...
{
  "id": "subsec-K-fold-cross-validation-7",
  "level": "2",
  "url": "sec-Model-Selection.html#subsec-K-fold-cross-validation-7",
  "type": "Example",
  "number": "2.8.6",
  "title": "",
  "body": " In this example we illustrate the use of cross-validation to pick the best model by fitting a noisy sinusoid by polynomilas with unknown best degree of the polynomial. Here, are the coefficients, which will be obtained by a linear regression algorithm and , the degree of polynomial of the model, is the hyperparameter. To find the best value of the hyperparameter we use the generalization error on the validation subset in a 5-fold cross-validation procedure. For error, we will use the square-root of the square error we have talked above by computing mean root mean squared error (RMSE) on the validation subsets of various folds. Note, the are the optimum values of the coefficients obtained by using the training dataset. Only after we have obtained the trained model, we compute the RMSE on the validation dataset. All of these steps can be implemented from scratch or you can use excellent standard libraries such as sklearn and PyTorch.  The program listed below first generates some data with noise. Then, we try to fit the data with polynomial regression models of varying degrees. The function cross_val_score(...) takes that model and performs the following steps for each fold:   Splits the data into training and validation sets (according to the KFold splitter).    Trains (fits) the model on the training set. Since, this model is just a linear regression, the fit is just one step. If we had a neural network, we would train for a number of epochs, i.e., cycles through the trianing subsets.    Evaluates it on the validation set using the chosen metric (in this case, negative MSE).    Repeats this across all folds.    The function then returns the list of validation scores (one per fold).      shows the result for different values of along the abscissa, each of those values correspond to a different model. This plot may produce a clear U-shaped curve as in this figure, but often can be very noisy. Here, low-degree polynomials (deg 1 and 2), have high bias and high error, clear indicators of underfit. The high-degree polynomials have high validation error, which is an indicator of overfit. The intermediate range has the best compromise with the least error.   Illustration of the use of cross-validation to select the model with the least generalization error. The data is fit with polynomials of different degrees by regression.       # This program is based on a program generated by GPT-5. import numpy as np import matplotlib.pyplot as plt from sklearn.preprocessing import PolynomialFeatures from sklearn.linear_model import LinearRegression from sklearn.pipeline import make_pipeline from sklearn.model_selection import cross_val_score, KFold plt.rcParams['axes.labelsize'] = 14 plt.rcParams['xtick.labelsize'] = 12 plt.rcParams['ytick.labelsize'] = 12 plt.rcParams['legend.fontsize'] = 12 # 1. Generate synthetic data np.random.seed(42) X = np.linspace(0, 5, 50).reshape(-1, 1) y = np.sin(X).ravel() + np.random.normal(scale=0.6, size=X.shape[0]) # noisy sine wave # 2. Hyperparameter: polynomial degree degrees = range(1, 11) cv_rmse = [] kf = KFold(n_splits=5, shuffle=True, random_state=42) # 3. Cross-validation for each degree for deg in degrees: model = make_pipeline(PolynomialFeatures(deg), LinearRegression()) scores = cross_val_score(model, X, y, cv=kf, scoring='neg_mean_squared_error') # implements the cross-validation rmse_mean = np.sqrt(-scores.mean()) cv_rmse.append(rmse_mean) print(f\"Degree={deg:2d} -> CV mean RMSE = {rmse_mean:.3f}\") # 4. Find best degree best_deg = degrees[np.argmin(cv_rmse)] best_rmse = min(cv_rmse) print(f\"\\nBest polynomial degree = {best_deg}, CV mean RMSE = {best_rmse:.3f}\") # 5. Plot plt.figure(figsize=(8,5)) plt.plot(degrees, cv_rmse, marker='o', color='blue') plt.axvline(best_deg, color='red', linestyle='--', label=f\"Best polynomial degree = {best_deg}\") plt.xlabel(\"Polynomial Degree\") plt.ylabel(\"Mean CV RMSE\") plt.xticks(degrees) plt.legend(loc=\"upper center\") plt.grid(True, linestyle='--', alpha=0.6) plt.tight_layout() plt.show()  "
},
{
  "id": "sec-Multivariate-Regression-Project",
  "level": "1",
  "url": "sec-Multivariate-Regression-Project.html",
  "type": "Section",
  "number": "2.9",
  "title": "A Multivariate Regression Project",
  "body": " A Multivariate Regression Project           "
},
{
  "id": "backmatter-2",
  "level": "1",
